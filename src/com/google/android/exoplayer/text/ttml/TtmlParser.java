...
public final class TtmlParser implements SubtitleParser {
	...
	private static final Pattern CLOCK_TIME = Pattern
	...
	/**
	 * @param strictParsing
	 *            If true, {@link #parse(InputStream)} will throw a
	 *            {@link ParserException} if the stream contains invalid data.
	 *            If false, the parser will make a best effort to ignore minor
	 *            errors in the stream. Note however that a
	 *            {@link ParserException} will still be thrown when this is not
	 *            possible.
	 */
						...
						if (!isSupportedTag(name)) {
							...
							try {
								...
								if (strictParsing) {
									...
									// Treat the node (and by extension, all of
									// its children) as unsupported.
									unsupportedNodeDepth++;
								}
							}
						}
	...
	private Map<String, TtmlStyle> parseHeader(XmlPullParser xmlParser, Map<String, TtmlStyle> globalStyles)
	...
	private TtmlStyle parseStyleAttributes(XmlPullParser parser, TtmlStyle style) {
		...
		for (int i = 0; i < attributeCount; i++) {
			...
			String sname = ParserUtil.removeNamespacePrefix(attributeName);

			if (sname.equals(TtmlNode.ATTR_ID)) {
				if (TtmlNode.TAG_STYLE.equals(parser.getName())) {
					style = createIfNull(style).setId(attributeValue);
				}
			} else if (sname.equals(TtmlNode.ATTR_TTS_BACKGROUND_COLOR)) {
			...
			} else if (sname.equals(TtmlNode.ATTR_TTS_COLOR)) {
				style = createIfNull(style);
				try {
			...
			} else if (sname.equals(TtmlNode.ATTR_TTS_FONT_FAMILY)) {
				style = createIfNull(style).setFontFamily(attributeValue);
			} else if (sname.equals(TtmlNode.ATTR_TTS_FONT_SIZE)) {

			} else if (sname.equals(TtmlNode.ATTR_TTS_FONT_WEIGHT)) {
				style = createIfNull(style).setBold(TtmlNode.BOLD.equalsIgnoreCase(attributeValue));
			} else if (sname.equals(TtmlNode.ATTR_TTS_FONT_STYLE)) {
				style = createIfNull(style).setItalic(TtmlNode.ITALIC.equalsIgnoreCase(attributeValue));
			} else if (sname.equals(TtmlNode.ATTR_TTS_TEXT_ALIGN)) {
				String sv = Util.toLowerInvariant(attributeValue);
				if (sv.equals(TtmlNode.LEFT)) {
					style = createIfNull(style).setTextAlign(Layout.Alignment.ALIGN_NORMAL);
				} else if (sv.equals(TtmlNode.START)) {
					style = createIfNull(style).setTextAlign(Layout.Alignment.ALIGN_NORMAL);
				} else if (sv.equals(TtmlNode.RIGHT)) {
					style = createIfNull(style).setTextAlign(Layout.Alignment.ALIGN_OPPOSITE);
				} else if (sv.equals(TtmlNode.END)) {
					style = createIfNull(style).setTextAlign(Layout.Alignment.ALIGN_OPPOSITE);
				} else if (sv.equals(TtmlNode.CENTER)) {
					style = createIfNull(style).setTextAlign(Layout.Alignment.ALIGN_CENTER);
				}
			} else if (sname.equals(TtmlNode.ATTR_TTS_TEXT_DECORATION)) {
				String sv = Util.toLowerInvariant(attributeValue);
				if (sv.equals(TtmlNode.LINETHROUGH)) {
					style = createIfNull(style).setLinethrough(true);
				} else if (sv.equals(TtmlNode.NO_LINETHROUGH)) {
					style = createIfNull(style).setLinethrough(false);
				} else if (sv.equals(TtmlNode.UNDERLINE)) {
					style = createIfNull(style).setUnderline(true);
				} else if (sv.equals(TtmlNode.NO_UNDERLINE)) {
...
//		}
	...
	private static boolean isSupportedTag(String tag) {
		if (tag.equals(TtmlNode.TAG_TT) || tag.equals(TtmlNode.TAG_HEAD) || tag.equals(TtmlNode.TAG_BODY)
				|| tag.equals(TtmlNode.TAG_DIV) || tag.equals(TtmlNode.TAG_P) || tag.equals(TtmlNode.TAG_SPAN)
				|| tag.equals(TtmlNode.TAG_BR) || tag.equals(TtmlNode.TAG_STYLE) || tag.equals(TtmlNode.TAG_STYLING)
				|| tag.equals(TtmlNode.TAG_LAYOUT) || tag.equals(TtmlNode.TAG_REGION)
				|| tag.equals(TtmlNode.TAG_METADATA) || tag.equals(TtmlNode.TAG_SMPTE_IMAGE)
	 ...
	 * <a href="http://www.w3.org/TR/ttaf1-dfxp/#timing-value-timeExpression">
	 * timeExpression</a>
	 *
	 * @param time
	 ...
	 * @param subframeRate
	 *            The sub-frame rate of the stream
	 * @param tickRate
	...
	private static long parseTimeExpression(String time, int frameRate, int subframeRate, int tickRate)
		...
		if (matcher.matches()) {
			...
			durationSeconds += (subframes != null) ? ((double) Long.parseLong(subframes)) / subframeRate / frameRate
					: 0;
